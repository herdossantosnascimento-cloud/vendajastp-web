    ...
    try {
      if (event.type === "checkout.session.completed") {
        ...
        if (paymentStatus !== "paid") {
          console.error("Checkout session not paid:", {
            uid,
            plan,
            paymentStatus,
            sessionId: session.id,
          });
          res.status(400).send("Checkout session not paid");
          return;
        }
